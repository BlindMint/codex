        ...
        viewModelScope.launch {
            try {
                ...
                if (bookWithCover != null) {
                    ...
                    _state.value = _state.value.copy(isDownloading = false, downloadProgress = 0f, error = "Failed to download book")
                }
            } catch (e: Exception) {
                _state.value = _state.value.copy(isDownloading = false, downloadProgress = 0f, error = "Download failed: ${e.message ?: "Unknown error"}")
            }
        }
    }

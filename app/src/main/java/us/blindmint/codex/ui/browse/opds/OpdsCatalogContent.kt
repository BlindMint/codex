                ...
                Row(
                    modifier = Modifier
                        ...
                        .noRippleClickable {
                            ...
                            android.util.Log.d(
                                "OPDS_DEBUG",
                                "Found navigation link: ${link?.let { "rel='${it.rel}', href='${it.href}'" } ?: "null"}")
                            ...
                            if (link != null) {
                                ...
                                try {
                                    ...
                                    android.util.Log.d(
                                        "OPDS_DEBUG",
                                        "navigator.push() completed successfully"
                                    )
                                } catch (e: Exception) {
                                ...
                                android.util.Log.d(
                                    "OPDS_DEBUG",
                                    "No navigation link found for '${entry.title}'"
                                )
                            }
                        }
            ...
            when {
                ...
                state.isDownloading -> Column(
                    ...
                    val currentBookIndex =
                        (state.downloadProgress * (state.selectedBooks.size)).toInt()
                ...
                state.error != null -> Column(
                    ...
                    val allCategories = state.feed?.entries?.filter { entry ->
                        ...
                        val isCategory =
                            hasNavigationLinks && !hasAcquisitionLinks && !hasSelfReferencingLinks

                        // Debug logging
                        if (entry.title?.contains("sub", ignoreCase = true) == true || isCategory) {
                            android.util.Log.d(
                                "OPDS_DEBUG",
                                "Entry '${entry.title}' - Navigation: $hasNavigationLinks, Acquisition: $hasAcquisitionLinks, SelfRef: $hasSelfReferencingLinks, IsCategory: $isCategory"
                            )
                    ...
                    android.util.Log.d(
                        "OPDS_DEBUG",
                        "Total entries: ${state.feed?.entries?.size ?: 0}"
                    )
